  ...
  handler: async (ctx, args) => {
    ...
    const moviesWithUrls = await Promise.all(
      movies.page.map(async (movie) => ({
        ...movie,
        thumbnail: movie.thumbnail ? await ctx.storage.getUrl(movie.thumbnail) || "" : "",
        videoUrl: movie.videoUrl ? await ctx.storage.getUrl(movie.videoUrl) || "" : "",
      }))
    );
...
export const getFeaturedMovie = query({
  ...
  handler: async (ctx) => {
    ...
    if (featuredMovie) {
      return {
        ...featuredMovie,
        thumbnail: featuredMovie.thumbnail ? await ctx.storage.getUrl(featuredMovie.thumbnail) || "" : "",
        videoUrl: featuredMovie.videoUrl ? await ctx.storage.getUrl(featuredMovie.videoUrl) || "" : "",
      };
    }
    ...
    if (latestMovie) {
      return {
        ...latestMovie,
        thumbnail: latestMovie.thumbnail ? await ctx.storage.getUrl(latestMovie.thumbnail) || "" : "",
        videoUrl: latestMovie.videoUrl ? await ctx.storage.getUrl(latestMovie.videoUrl) || "" : "",
      };
    }
...
export const getMovieById = query({
  ...
  handler: async (ctx, args) => {
    ...
    const thumbnailUrl = movie.thumbnail ? await ctx.storage.getUrl(movie.thumbnail) : null;
